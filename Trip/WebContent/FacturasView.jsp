	...
	<shiro:user>
		...
		<div class="container ">
			<div class="panel-body "
				style="margin-top: 75px; margin-left: 5%; margin-right: 5%">
				<table class="table table-striped table-bordered table-list "
					style="text-align: center;">
					<thead>
						<tr>
							<th class="hidden-xs">Número de viaje</th>
					...
					<c:forEach items="${facturas}" var="facturai">
						<tbody>
							<tr>
								<td class="hidden-xs align-middle">${facturai.viaje.nViaje}</td>
								<td class="align-middle">${facturai.nFactura }</td>
								<td>
									<form action="ServeFileFacturasServlet" method="get">
										<input type="hidden" name="nFactura"
											value="${facturai.nFactura}" />
				...
				<button class="navbar-toggler" type="bottom" data-toggle="collapse"
					...
					<form action="HomeEncargadoServlet" method="get">
						<input type="hidden" name="email"
							value="${viaje.viajero.superior.email }" />
						<button class="btn" type="submit">Volver</button>
					</form>
				</button>
